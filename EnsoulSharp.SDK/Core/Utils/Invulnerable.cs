// <copyright file="Invulnerable.cs" company="EnsoulSharp">
                ...
                new List<InvulnerableEntry>
                    {
                        new InvulnerableEntry("FerociousHowl")
                            { ChampionName = "Alistar", CheckFunction = (target, type) => GameObjects.Player.CountEnemyHeroesInRange(GameObjects.Player.GetRealAutoAttackRange()) > 1 },
                        ...
                        new InvulnerableEntry("SivirE")
                            { ChampionName = "Sivir", IsShield = true },
                        new InvulnerableEntry("TaricR")
                            { },
                        new InvulnerableEntry("UndyingRage")
                            { ChampionName = "Tryndamere", CheckFunction = (target, type) => target.Health <= new []{ 30, 50, 70 }[Math.Max(0, target.Spellbook.GetSpell(SpellSlot.R).Level - 1)] },
                        new InvulnerableEntry("XinZhaoRRangedImmunity")
                            { ChampionName = "XinZhao", DamageType = DamageType.Physical },
