// <copyright file="OrbwalkerBase.cs" company="EnsoulSharp">
...
namespace EnsoulSharp.SDK
{
        ...
        where TK : struct, IConvertible where T : AttackableUnit
    {
        ...
        public virtual bool Enabled
        {
            ...
            set
            {
                if (this.enabled != value)
                {
                    if (value)
                    {
                        AIBaseClient.OnProcessSpellCast += this.OnAIBaseClientProcessSpellCast;
                        AIBaseClient.OnDoCast += this.OnAIBaseClientDoCast;
                        AIBaseClient.OnBuffGain += this.AIBaseClientOnOnBuffAdd;
        ...
        ///     The event data
        /// </param>
        private void OnAIBaseClientProcessSpellCast(AIBaseClient sender, AIBaseClientProcessSpellCastEventArgs args)
        {
            if (sender.IsValid && sender.IsMe)
            {
                if (Game.Ping <= 30)
                {
                    DelayAction.Add(30, () => this.OnAIBaseClientProcessSpellCastDelayed(sender, args));
                }
                else
                {
                    this.OnAIBaseClientProcessSpellCastDelayed(sender, args);
                }
            }
        }

        /// <summary>
        ///     Delayed OnProcessSpellCast event.
        /// </summary>
        /// <param name="sender">
        ///     The sender
        /// </param>
        ...
        ///     The event data
        /// </param>
        private void OnAIBaseClientProcessSpellCastDelayed(AIBaseClient sender, AIBaseClientProcessSpellCastEventArgs args)
        {
        ...
        ///     The event data
        /// </param>
        private void OnAIBaseClientDoCast(AIBaseClient sender, AIBaseClientProcessSpellCastEventArgs args)
        {
            if (sender.IsValid && sender.IsMe)
            {
