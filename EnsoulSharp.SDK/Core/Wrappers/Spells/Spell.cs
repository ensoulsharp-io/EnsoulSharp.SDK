// <copyright file="Spell.cs" company="EnsoulSharp">
...
namespace EnsoulSharp.SDK
{
    ...
    public class Spell
    {
        ...
        public Vector3 From
        {
            get
            {
                return !this.@from.ToVector2().IsValid() ? GameObjects.Player.Position : this.@from;
            }
        ...
        ///     Gets the instance.
        /// </summary>
        public SpellDataInstClient Instance => GameObjects.Player.Spellbook.GetSpell(this.Slot);
            ...
            HitChance tempHitChance = HitChance.None)
        {
            ...
            if (!this.IsSkillshot)
            {
                if (this.RangeCheckFrom.DistanceSquared(unit.Position) > this.RangeSqr)
                {
        ...
        ///     Was Spell Casted
        /// </returns>
        public bool CastOnUnit(AIBaseClient unit)
        {
            if (!this.IsReady() || this.From.DistanceSquared(unit.Position) > this.RangeSqr)
            {
                return false;
            }
        ...
        ///     The hits
        /// </returns>
        public int CountHits(List<AIBaseClient> units, Vector3 castPosition)
        {
        ...
        /// </returns>
        public FarmLocation GetCircularFarmLocation(List<AIBaseClient> minions, float overrideWidth = -1)
        {
        ...
        ///     Collision List
        /// </returns>
        public List<AIBaseClient> GetCollision(Vector2 fromVector2, List<Vector2> to, float delayOverride = -1)
        {
        ...
        ///     Farm Location. <seealso cref="FarmLocation" />
        /// </returns>
        public FarmLocation GetLineFarmLocation(List<AIBaseClient> minionPositions, float overrideWidth = -1)
        {
            var positions = Minion.GetMinionsPredictedPositions(
                minionPositions,
                this.Delay,
        ...
        ///     provided.
        /// </returns>
        public IEnumerable<AIBaseClient> GetUnitsByHitChance(HitChance minimumHitChance = HitChance.High)
        {
            return
                GameObjects.Enemy.Where(
                    unit => this.WillHit(unit, GameObjects.Player.Position, 0, minimumHitChance));
        }
        ...
        public bool IsInRange(GameObject obj, float otherRange = -1)
        {
            return this.IsInRange(
                (obj as AIBaseClient)?.Position.ToVector2() ?? obj.Position.ToVector2(),
                otherRange);
        }
